...
Функция ПечатьЦенников()
	...
	|	1 КАК Количество
	|ИЗ
	|	Документ.ПриходныйОрдерНаКруглыйЛесоматериал.Товары КАК Док
	|ГДЕ
	|	Док.Ссылка = &Док
	|");
...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Иначе
		...
		Если НаПринтер Тогда
			УправлениеПечатьюКлиент.ВыполнитьКомандуПечатиНаПринтер("Документ.ПриходныйОрдерНаКруглыйЛесоматериал", ИмяМакета, 
										ПараметрКоманды, Неопределено);
		Иначе
			УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.ПриходныйОрдерНаКруглыйЛесоматериал", ИмяМакета, 
	...
	Пока Сч < ТаблицаПоТоварамРезерв.Количество() Цикл
		СтрокаТаблицы = ТаблицаПоТоварамРезерв.Получить(Сч);
			Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ДокументРезерва) 
			 ИЛИ (ТипЗнч(СтрокаТаблицы.ДокументРезерва) = Тип("ДокументСсылка.ПриходныйОрдерНаКруглыйЛесоматериал") 
...
Функция ПодготовитьТаблицуДляТоварамВРезервеПоОрдерам(ТабТовары)
	...
	Пока Сч < ТаблицаПоТоварамКПолучению.Количество() Цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ДокументРезерва) ИЛИ ТипЗнч(СтрокаТаблицы.ДокументРезерва) <> Тип("ДокументСсылка.ПриходныйОрдерНаКруглыйЛесоматериал") Тогда
...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам,
	                          ТаблицаПоТаре, ТаблицаПоШтабелям, Отказ, Заголовок);
...
//{{MRG[ <-> ]
	//KIT
//}}MRG[ <-> ]
	Если ВидОперации = Перечисления.ВидыОперацийПриходныйОрдер.НеОпределен И ОтвественноеХранение Тогда
	...
	// ТОВАРЫ ПО РЕГИСТРУ ПиломатериалыОбъектовУчетаЛФ, ТоварыНаСкладах, ТоварыОрганизаций, ТоварыКПолучениюНаСклады
	Если ОтгрузкаНЗП Тогда
		Если ДокументОснование.ОтгрузкаНаПромсклад И ДокументОснование.ОбъектУчетаЛесногоФондаПромсклад.Код =  "22В000742" Тогда
			//31012018  сторно запись убрать
			НаборЗаписей = РегистрыНакопления.ПиломатериалыОбъектовУчетаЛФ.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.Регистратор.Установить(ДокументОснование.Ссылка);
			НаборЗаписей.Записать();

			РасходыйОрдерОбъект =  ДокументОснование.ПолучитьОбъект();
			РасходыйОрдерОбъект.Записать(РежимЗаписиДокумента.Проведение);
			//НаборЗаписей = РегистрыНакопления.ПиломатериалыОбъектовУчетаЛФ.СоздатьНаборЗаписей();
			//НаборЗаписей.Отбор.Регистратор.Установить(ДокументОснование.Ссылка);
			//НаборЗаписей.Прочитать();
			//НаборДвижений = Движения.ПиломатериалыОбъектовУчетаЛФ;
			//Для каждого СтрокаНЗ из НаборЗаписей Цикл
			//	Если СтрокаНЗ.ВидДвижения = ВидДвиженияНакопления.Приход Тогда 
			//		ЗаписьДвижения = НаборДвижений.Добавить();
			//		ЗаполнитьЗначенияСвойств(ЗаписьДвижения,СтрокаНЗ);
			//		ЗаписьДвижения.Количество = -СтрокаНЗ.Количество; 
			//	КонецЕсли;
			//КонецЦикла;
			//НаборДвижений.Записать();
			
			НаборДвиженийТоварыНаСкладах = Движения.ТоварыНаСкладах;
			НаборДвиженийТоварыНаСкладах.Очистить();
			НаборДвиженийТоварыНаСкладах.Записать();
			НаборДвиженийТоварыОрганизаций =  Движения.ТоварыОрганизаций;
			НаборДвиженийТоварыОрганизаций.Очистить();
			НаборДвиженийТоварыОрганизаций.Записать();
			НаборДвиженийТоварыКПолучениюНаСклады = Движения.ТоварыКПолучениюНаСклады;
			НаборДвиженийТоварыКПолучениюНаСклады.Очистить();
			НаборДвиженийТоварыКПолучениюНаСклады.Записать();
		КонецЕсли;
	КонецЕсли;
	
	Если СобственныйПиловочник Тогда 
		РасходыйОрдерОбъект =  ДокументОснование.ПолучитьОбъект();
		РасходыйОрдерОбъект.Записать(РежимЗаписиДокумента.Проведение);
	КонецЕсли;
	
	УчетШтабелейВключен = Ложь;
	Если УчетШтабелейВключен  Тогда 
		ТаблицаПоШтабелям.Колонки.Добавить("ВидДвижения");
		ТаблицаПоШтабелям.ЗаполнитьЗначения(ВидДвиженияНакопления.Приход, "ВидДвижения");
		
		Движения.ОстаткиШтабелейKIT.Загрузить(ТаблицаПоШтабелям);
		Движения.ОборотШтабелейKIT.Загрузить(ТаблицаПоШтабелям);
	КонецЕсли;
...
Процедура ДвиженияПоРегистрамРегл(РежимПроведения, СтруктураШапкиДокумента, 
	...
	Для Каждого СтрТаблица из ТаблицаПоТоварам Цикл
		ПроводкиБУ = Движения.Хозрасчетный.ДобавитьДебет();
		ПроводкиБУ.СчетДт = ПланыСчетов.Хозрасчетный.ТМЦпринятыеНаОтветственноеХранение;
		БухгалтерскийУчет.УстановитьСубконто(ПроводкиБУ.СчетДт,ПроводкиБУ.СубконтоДт,"Контрагенты",ЭтотОбъект.Контрагент);
		БухгалтерскийУчет.УстановитьСубконто(ПроводкиБУ.СчетДт,ПроводкиБУ.СубконтоДт,"Номенклатура",СтрТаблица.Номенклатура);
		БухгалтерскийУчет.УстановитьСубконто(ПроводкиБУ.СчетДт,ПроводкиБУ.СубконтоДт,"Склады", ЭтотОбъект.Склад);
		ПроводкиБУ.КоличествоДт = СтрТаблица.Количество;
...
Процедура ОбработкаЗаполнения(Основание, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("СправочникСсылка.НастройкиЗаполненияФорм") Тогда
		...
		ХранилищаНастроек.ДанныеФорм.ЗаполнитьОбъектПоНастройке(ЭтотОбъект, Основание, Документы.ПриходныйОрдерНаКруглыйЛесоматериал.СтруктураДополнительныхДанныхФормы());
...
Процедура ПриЗаписи(Отказ)
	...
	//kit
	Если ОтгрузкаНЗП И Не ЗначениеЗаполнено(ДокументОснование) Тогда
		Отказ = Истина;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	СтруктураПолей.Вставить("Штабель"              		, "Штабель");
	СтруктураПолей.Вставить("ТС"                		, "Ссылка.Машина");
	СтруктураПолей.Вставить("Количество"                , "Количество");

	РезультатЗапросаШтабели = УправлениеЗапасами.СформироватьЗапросПоТабличнойЧасти(ЭтотОбъект, "Замеры", СтруктураПолей);
	ТаблицаПоШтабелям = ПодготовитьТаблицуШтабелей(РезультатЗапросаШтабели, СтруктураШапкиДокумента);
	...
	Если Не Отказ Тогда
		...
		ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, 
							ТаблицаПоТоварам, ТаблицаПоТаре, ТаблицаПоШтабелям,
							Отказ, Заголовок);
...
//  Сформированная таблица значений.
//
Функция ПодготовитьТаблицуШтабелей(РезультатЗапросаПоШтабелям, СтруктураШапкиДокумента)

	ТаблицаШтабелей = РезультатЗапросаПоШтабелям.Выгрузить();

	Возврат ТаблицаШтабелей;
