...
Процедура ПечатьРеестр(ТабДок, Ссылка) Экспорт
	...
	Макет = Документы.УкладкаВШтабель.ПолучитьМакет("ПечатьРеестр");
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	УкладкаВШтабель.Номер,
		|	УкладкаВШтабель.Дата,
		|	УкладкаВШтабель.Организация,
		|	УкладкаВШтабель.Подразделение,
		|	УкладкаВШтабель.Автор,
		|	УкладкаВШтабель.Состав.(
		|		НомерСтроки,
		|		Номенклатура,
		|		Порода,
		|		Сорт,
		...
		|		СкладПолучатель)
		|ИЗ
		|	Документ.УкладкаВШтабель КАК УкладкаВШтабель
		|ГДЕ
		|	УкладкаВШтабель.Ссылка В (&Ссылка)";
